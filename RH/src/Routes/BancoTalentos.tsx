  ...
  const [candidates, setCandidates] = useState<TCandidate[]>([
    {
      ...
      "descricaoATVD": "Desenvolvimento de sistemas e manutenção de infraestrutura de TI.",
    ...
    },
    {
      ...
      "descricaoATVD": "Gestão de campanhas publicitárias e estratégias de marketing digital.",
      "situacao": "Empregada",
    ...
    },
    {
      ...
      "descricaoATVD": "Consultoria para aumento de vendas e expansão de mercado.",
      "situacao": "Empregada",
      "escolaridade": "Ensino Superior Completo",
      "curso": "Administração",
  ...
  return (
    <div className="bg-white w-full rounded-lg shadow-md shadow-gray-200 p-5 flex flex-col gap-5 ">
      ...
      <div className="overflow-x-scroll ">
            ...
            {candidates && candidates.length > 0 ? (
              candidates?.map((candidate) => (
                <tr className="border-t mt-10 hover:bg-gray-100">
                  ...
                  onClick={() => { setIsPopUpOpen(!isPopUpOpen); setCurrentID(candidate.id) }}>
                    <p className="bg-gray-500 text-white cursor-pointer w-fit p-1 px-3 rounded-xl">Ver currículo completo</p>
                  </td>
                </tr>
              ))
            ) : (
        ...
        {
          isPopUpOpen && (
            <div className="bg-black/20 w-screen h-screen absolute top-0 left-0 flex flex-col items-center justify-center">
              <div className=" bg-white  shadow-lg rounded-xl p-10">
                <div className="flex justify-end text-3xl cursor-pointer" onClick={() => setIsPopUpOpen(!isPopUpOpen)}>x</div>
                {candidates.map((candidate) => {
                  if (candidate.id === currentID) {
                    return (
                      <CurriculoLayout candidate={candidate}/>
                    );
                  }
                  return null;
                })}
